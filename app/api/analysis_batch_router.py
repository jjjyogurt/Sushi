...
def get_analysis_batch(batch_id: int, db: Session = Depends(get_db_session)):
    service = AnalysisBatchService(db)
    try:
        return map_analysis_batch_response(service.get_batch(batch_id))
    except ValueError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error


@router.get("/{batch_id}/items", response_model=AnalysisBatchDetailResponse)
def get_analysis_batch_items(batch_id: int, db: Session = Depends(get_db_session)):
    service = AnalysisBatchService(db)
    try:
...
def get_latest_active_batch(
    ...
    service = AnalysisBatchService(db)
    batch = service.get_latest_active_batch(monitor_profile_id=monitor_profile_id)
    if batch is None:
...
def cancel_analysis_batch(batch_id: int, db: Session = Depends(get_db_session)):
    service = AnalysisBatchService(db)
    try:
        return map_analysis_batch_response(service.cancel_batch(batch_id))
    except ValueError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
