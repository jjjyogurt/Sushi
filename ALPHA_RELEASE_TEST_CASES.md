...
| VIDEO-DISC-001 | User Keyword Discovery With Publish Window | User is signed in; project has user-defined brand keywords and key products; YouTube discovery can be mocked or run against a controlled fixture; optional SerpAPI key can be enabled for market-local manual discovery QA | Open the project video queue -> choose a publish window such as last 24 hours or last 7 days -> click Discover Videos -> inspect `/videos/discover` payload and resulting queue | Discovery sends `max_results=50`, the selected time trigger, and publish window; when SerpAPI is configured, manual discovery uses SerpAPI as the primary market-local candidate source, validates returned video IDs through YouTube Data API `videos.list`, filters by canonical `publishedAt`, then supplements with YouTube Data API latest search; each YouTube Data API search query requests `maxResults=50` instead of dividing the origin fetch across queries; searches exact user-defined keywords across configured languages without localized/Gemini query variants; when English and non-English languages are both configured, searches the configured non-English languages first and keeps English as fallback within the three-language cap; saves relevant candidates when the keyword appears in title or description; keeps brand-level videos even without exact key-product text; loosely filters obvious CJK-title noise when the project does not include CJK languages; and does not create alerts or analysis results by itself | Unit/API + Browser Manual |
...
| VIDEO-DETAIL-001 | Video Detail Metadata Labels | User is signed in and a project video is selected on `/projects/{id}` | Open the selected video detail header at desktop and phone-width viewports | Views, influencer subscribers, analysis status, and analysis language render as consistent label/value pills; the language pill is labeled as analysis language rather than detected video language; published date remains secondary metadata; pills wrap without overlapping actions or title text | Unit + Browser Manual |
| VIDEO-TRANSCRIPT-001 | Bilingual Transcript Display + Download | User is signed in and a project has a completed analysis with English and `zh-Hans` rows | Open video detail -> inspect Transcript toolbar -> click Download in English -> switch to `中文` -> click Download again; inspect `/videos/{id}/analysis?language=en` and `?language=zh-Hans` responses | Download appears beside Expand; empty transcripts cannot be downloaded; English download contains English transcript metadata/content when available, Chinese download contains Chinese transcript metadata/content when available, filenames include YouTube ID and language, and transcript provenance/status fields show source language plus translation status without stale cross-language text. If only row-language transcript translation fails, analysis still renders and the transcript panel shows a transcript translation warning instead of “No analysis yet.” | Unit/API + Browser Manual |
| VIDEO-TRANSCRIPT-002 | Japanese Source Transcript Does Not Block Analysis | User is signed in, local backend has been restarted after code changes, and a project has a Japanese-caption video such as project 6 video `hSu6T2Vm1cM` | Verify the local server process was started from the current code revision with reload or a fresh restart -> open the video detail -> click Re-run Analysis -> wait for terminal state -> inspect English and `zh-Hans` analysis responses -> expand Transcript -> click Download for English and Chinese | English analysis completes using the source Japanese transcript; transcript translation uses one bilingual Gemini call for English and Simplified Chinese; tagged transcript responses, valid JSON responses, and JSON-like multiline transcript responses parse into available transcripts; transcript-only failures appear in `transcript_status`/`transcript_error_message`; the video detail does not show `Analysis status: Failed` for transcript translation-only errors; Japanese transcript rows show timestamp/sentence layout and downloads include selected-language transcript content; stale no-reload backend processes are treated as invalid test setup; strict timestamp line-count mismatch is not a failure condition | Unit/API + Browser Manual |
