...
class VideoCandidate(TimestampMixin, Base):
    ...
    discovery_source: Mapped[str] = mapped_column(String(40), default="manual", nullable=False)
    discovered_by_monitor_run_id: Mapped[Optional[int]] = mapped_column(nullable=True)
    proactive_seen_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
